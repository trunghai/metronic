...
Combo.prototype.show = function (idParent){
    ...
    container.innerHTML = renderDataCombo(this.accountName,this.accountNumber,this.accountBalance,this.paddingIcon, this.hiddenArrow);
...
function renderDataCombo(accountName, accountNumber,accountBalance,paddingIcon, hiddenArrow){
    ...
    if(typeof accountBalance !== undefined && accountBalance.length > 0 && accountBalance!=""){
        html+='<div class="accName"><span>'+CONST_STR.get('ACCOUNT_AVAILABLE_BALANCE')+'</span></div><div class="accountBalance"><span>'+accountBalance+'</span><span> VND</span></div>';
    }
    html+='</div>';
    if (!hiddenArrow){
