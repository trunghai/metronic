...
function initData() {
    angular.module('EbankApp').controller('auth-search-authen', function ($scope, requestMBServiceCorp) {
        ...
        $scope.authorizationTrans = function () {
            ...
            if (gTrans.authen){
                ...
                for (var i in gTrans.listSelectedTrans){
                    gTrans.transInfo.push({'transId': gTrans.listSelectedTrans[i].MA_GD, 'userIdRef': gTrans.listSelectedTrans[i].IDUSERREFERENCE});
                }
                request.sequenceId = 2;
                request.idtxn = "S65";
                request.transIds= gTrans.transInfo,
                request.transType= gTrans.listSelectedTrans.IDTXN;
                ...
                requestMBServiceCorp.post(data, true, function (response){
                    gPayment.result = {};
                    gPayment.result.message = response.respContent;
                    gPayment.result.respJsonObj = response.respJsonObj;
					gTrans.listTrans=response.respJsonObj.table
                    ...
                    navCachedPages["authorize/setup/auth-search-result"] = null;
                    navController.pushToView("authorize/setup/auth-search-result", true);
                ...
                for (var i in gTrans.listSelectedTrans){
                    if (request.transIds === undefined || request.transIds === null){
                        request.transIds = gTrans.listSelectedTrans[i].MA_GD;
                    }else {
                        request.transIds = request.transIds + ',' + gTrans.listSelectedTrans[i].MA_GD;
                    }
                }
        ...
        function handleOTPTimeout() {
            ...
            clearOTPTimeout();
            if (currentPage == "authorize/setup/auth-search-authen"){
