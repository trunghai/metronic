...
function init() {
    angular.module('EbankApp').controller('auth-trans-domestic', function ($scope, requestMBServiceCorp) {
        ...
        $scope.showTransTypeSelection =function () {
            var tmpArray1 = (gUserInfo.lang == 'EN') ? CONST_TRANS_TYPE_APPROVE_EN : CONST_TRANS_TYPE_APPROVE_VN;
            var tmpArray2 = CONST_TRANS_TYPE_APPROVE_ID;
        ...
        function showTransTypeSelectionOpen(e) {
            if (currentPage == "authorize/transfer/search-pending/domestic/transfer-domestic") {
        ...
        function requestInfoDetailTrans(IDUSERREFERENCE) {
            ...
            requestMBServiceCorp.post(data, true, function (response) {
                if(response.respCode == '0'){
                    ...
                    gTrans.transInfoRequest.idtxn = "T63";
                    ...
                    gTrans.idtxn = 'T63';
                    gTrans.transInfo.typeTrans = CONST_STR.get('COM_TRANS_INTER_BANK');
                    gTrans.transInfo.SEND_METHOD = getSendMethod(gTrans.transInfo.SEND_METHOD);
                    gTrans.transInfo.TYPE_TEMPLATE = getTransTempInfo(gTrans.transInfo.TYPE_TEMPLATE);
                    gTrans.cmdType = CONSTANTS.get('CMD_CO_AUTHORIZE_DOMESTIC_TRANSFER');
                    gTrans.scr = 'pages/authorize/transfer/search-pending/domestic/views/auth-domestic-info-view.html';
                    gTrans.srcViewListPending = 'authorize/transfer/search-pending/domestic/transfer-domestic';
        ...
        $scope.authorizeTransaction = function () {
            ...
            gTrans.obj.sequenceId = 8;
            gTrans.obj.idtxn = 'T63';
            ...
            gTrans.cmdType = CONSTANTS.get('CMD_CO_AUTHORIZE_DOMESTIC_TRANSFER');
            gTrans.srcViewListPending = 'authorize/transfer/search-pending/domestic/transfer-domestic';
            gTrans.srcAuthenDesktop = 'pages/authorize/transfer/search-pending/domestic/views/auth-domestic-review-destop.html';
            gTrans.srcAuthenMobile = 'pages/authorize/transfer/search-pending/domestic/views/auth-domestic-review-mobile.html';
...
function exportExcelDebtHistory(){
    ...
    var transIds = "";
    for (var i in gTrans.listPending) {
        transIds += gTrans.listPending[i].IDFCATREF + ",";
    }
