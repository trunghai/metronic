...
function init(){
    angular.module("EbankApp").controller("cre-guarantee-suggest-scr-auth", function ($scope, requestMBServiceCorp) {
        $scope.gTrans = gTrans;
        ...
        $scope.sendJSONAuthenRequest = function () {
            ...
            var request = {
                ...
                request: {
                    transId:gTrans.idcafcatref,
                    sequence_id:"3",
                    idtxn:"C10",
                    noteProfile : gTrans.transInfo.instructionFile,
                    disbursementType: gTrans.disbursementType,
                    customerNo: gTrans.customerNo,
            ...
            requestMBServiceCorp.post(data, true, function (response) {
                gTrans.result = {};
                gTrans.result.message = CONST_STR.get("MESSAGE_GUARANTEE");
                gTrans.result.respJsonObj = response.respJsonObj;
                gTrans.result.respJsonObj.status = CONST_STR.get("COM_TRANS_STATUS_INT");
                gTrans.result.respJsonObj.name = gUserInfo.accountName;
...
//                    navController.pushToView('common/common-result/transfer-result-scr', true, 'html');
                    navCachedPages["credit/guarantee/cre-guarantee-suggest-scr-final"] = null;
                    navController.pushToView("credit/guarantee/cre-guarantee-suggest-scr-final", true, "html");
                }else {
