...
var atmTpbArray;
//var atmTpbResultArray;
var atmTpbResultArray = new Array();
var atmCityCode = '';
var atmTpbFilterArray = new Array();
...
function initBankInfoTPBATM() {
	...
	if(!(navCheckPageCachedStatus('bankinfo/atm-info'))) {
		getAtmTpbList();
	}
	else {
		atmTpbResultArray = atmTpbArray;
		parserViewBankList();
		genBankListView();
		/*searchBankName();*/
	}
...
function showCitySelection(inArr) {
	var tmpArray1 = [];
	var tmpArray2 = [];
...
function showAreaSelection(inArr) {
	var tmpArray1 = [];
	var tmpArray2 = [];
...
function handleSelectionCityList(e) {
	if (currentPage == "bankinfo/atm-info") {
		...
		if ((e.selectedValue1 != undefined) && (e.selectedValue1 != null)) {
			...
			if (tagcity.nodeName == "INPUT") {
				...
				parserViewBankList();
				atmCityCode = e.selectedValue2;
				for (var i=0; i<atmTpbResultArray.length; i++) {
					if (atmTpbResultArray[i].split("#")[0] != atmCityCode)
					{
						atmTpbResultArray.splice(i,1);
						i--;
					}
				}
	...
	if (currentPage == "bankinfo/atm-info") {
		...
		if ((e.selectedValue1 != undefined) && (e.selectedValue1 != null)) {
			...
			if (tagarea.nodeName == "INPUT") {
				...
				parserViewBankList();
				for (var i=0; i<atmTpbResultArray.length; i++) {
					if (atmTpbResultArray[i].split("#")[1] != e.selectedValue2)
					{
						atmTpbResultArray.splice(i,1);
						i--;
					}
...
function genBankListView() {
	...
	if((atmTpbResultArray == null) || (atmTpbResultArray == undefined)) {
...
function getBankAtIndex(inIdx) {
	...
	gBankInfoATMSelected = atmTpbResultArray[inIdx]; //save bank info raw data
